...
//  - cold: fresh WebAssembly.Instance per command (uutils multicall) — the
//    demo's stand-in for fork+exec
//  - hot: a wasip1 reactor instantiated once; commands are function calls
//    (the M3 execution model, previewed)
...
export function createRuntime(wasmModule, hotModule = null) {
  ...
  // per-call state; swapped before each run (cold or hot)
  let ctx = null;
  const freshCtx = (stdinBytes) => ({
    stdin: stdinBytes ?? new Uint8Array(0),
    stdinPos: 0,
    stdout: [],
    stderr: [],
    nextFd: 5,
    fds: new Map([[3, { path: "", dir: true, preopen: "/" }]]),
  });
  let mem = null; // current instance's memory
  const view = () => new DataView(mem.buffer);
  const bytes = (p, l) => new Uint8Array(mem.buffer, p, l);
  ...
  const impl = {
    ...
    path_open(d, df, pP, pL, oflags, rB, rI, fdflags, outP) {
      ...
      const fd = ctx.nextFd++;
      ctx.fds.set(fd, entry.dir ? { path: p, dir: true } : { path: p, pos: fdflags & 1 ? entry.data.length : 0 });
    ...
    fd_read(fd, iovsP, iovsLen, nP) {
      ...
      const data = fd === 0 ? ctx.stdin : store.get(f.path)?.data ?? new Uint8Array(0);
      let pos = fd === 0 ? ctx.stdinPos : f.pos;
      let total = 0;
      ...
      if (fd === 0) ctx.stdinPos = pos; else f.pos = pos;
    ...
    fd_close(fd) { ctx.fds.delete(fd); return E.OK; },
    fd_sync: () => E.OK,
  ...
  const imports = { wasi_snapshot_preview1: new Proxy(impl, { get: (t, n) => t[n] ?? (() => E.NOSYS) }) };

  function runCold(argv, stdinBytes) {
    ctx = freshCtx(stdinBytes);
    args = [argv[0], ...argv];
  ...
  const HOT = new Set(["true", "false", "echo", "cat", "tee", "wc", "seq", "head", "sort", "grep"]);
  let hot = null;
  function runHot(argv, stdinBytes) {
    if (!hot) {
      ctx = freshCtx(null);
      const inst = new WebAssembly.Instance(hotModule, imports);
      mem = inst.exports.memory;
  ...
  function run(argv, stdinBytes) {
    if (hotModule && HOT.has(argv[0])) return runHot(argv, stdinBytes);
    return runCold(argv, stdinBytes);
  }

  const seedFile = (p, data) => store.set(p, { data, mtime: Date.now() });
  const seedDir = (p) => store.set(p, { dir: true, mtime: Date.now() });
  return { run, runCold, store, seedFile, seedDir, enc, dec };
}
